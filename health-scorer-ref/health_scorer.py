...
def build_genome() -> dict:
    """Build scorer genome with stateful health scorer using recent TTFT and preemption detection."""
    custom_source = '''
def health_scorer(workers, request, metrics_store, kv_index, cycle_state, params,
                  _state={"ttft_hist": {}, "prev_pre": {}}):
    """Stateful scorer: KV preemption risk (cubic) + recent-window TTFT +
    preemption delta detection. Targets p90 by avoiding degraded workers."""
    scores = {}
    hist = _state["ttft_hist"]
    pp = _state["prev_pre"]
    kv_thresh = 0.85
    window = 5
    ...
    for w in workers:
        ...
        if m is None or m.ttft_count <= 0:
            continue
        ring = hist.get(w.address)
        if ring is None:
            ring = []
            hist[w.address] = ring
        if not ring or ring[-1][1] != m.ttft_count:
            if ring and (m.ttft_sum < ring[-1][0] or m.ttft_count < ring[-1][1]):
                ring.clear()
            ring.append((m.ttft_sum, m.ttft_count))
            if len(ring) > window:
                hist[w.address] = ring[-window:]

    recent = {}
    for w in workers:
        ring = hist.get(w.address)
        if ring and len(ring) >= 2:
            ds = ring[-1][0] - ring[0][0]
            dc = ring[-1][1] - ring[0][1]
            if dc > 0 and ds >= 0:
                recent[w.address] = ds / dc
    mt = max(recent.values()) if recent else 1.0
    if mt <= 0:
        mt = 1.0
    ...
    for w in workers:
        ...
        rt = recent.get(w.address)
        ttft_score = 1.0 - rt / mt if rt is not None else 0.5
        scores[w.address] = 0.40 * kv_score + 0.35 * ttft_score + 0.25 * pre_score
    return scores
'''
    return {
        "scorers": [
            {"name": "precise_prefix_cache", "weight": 3.5, "params": {}},
            {"name": "no_hit_lru", "weight": 1.0, "params": {}},
            {"name": "active_request", "weight": 1.5, "params": {}},
            {"name": "health_scorer", "weight": 3.2, "params": {}},
        ],
        "custom_scorers": {
...
def run_experiment(**kwargs: object) -> dict:
    ...
    Returns dict with p90_trajectory_time, success_rate, etc.
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:  # noqa: S310
            if resp.status != 200:
                return {"p90_trajectory_time": 9999, "success_rate": 0}
    ...
    p90_traj_time = traj_time.get("p90", 9999)
    ...
    return {
        "p90_trajectory_time": p90_traj_time,
